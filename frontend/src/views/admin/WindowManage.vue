<template>
  <div>
    <div class="toolbar">
      ...
      <el-input v-model="keyword" placeholder="搜索档口名称" clearable style="width: 220px" @keyup.enter="load" @clear="load" />
    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="食堂" width="120">
        <template #default="{ row }">{{ row.canteen?.name || '-' }}</template>
      </el-table-column>
...
import { adminWindows, approveWindow, deleteWindow } from '@/api'
...
const load = async () => {
  loading.value = true
  try {
    const res = await adminWindows({ status: status.value || null, keyword: keyword.value || null, page: page.value, size })
...
const approve = async (row, s) => {
  await approveWindow(row.id, s)
  ElMessage.success(s === 'PUBLISHED' ? '已通过' : '已驳回')
  load()
}
const remove = async (row) => {
  await ElMessageBox.confirm(`确定删除档口「${row.name}」吗？其下菜品也会被删除`, '提示', { type: 'warning' })
