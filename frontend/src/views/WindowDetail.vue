    ...
    <div class="page-container">
      ...
      <template v-if="win">
        ...
        <div class="guat-card detail-card">
          <div class="cover">
            <img v-if="win.coverImage" :src="win.coverImage" alt="cover" />
            <el-icon v-else class="cover-placeholder"><Food /></el-icon>
          </div>
          <div class="info">
            <div class="title-row">
              <h2>{{ win.name }}</h2>
              <el-tag v-if="win.canteen" type="primary" effect="light">{{ win.canteen.name }}</el-tag>
            </div>
            <div class="desc">{{ win.description || '暂无简介' }}</div>
            <div class="loc"><el-icon><Location /></el-icon>{{ win.canteen?.location }} · {{ win.location || '未知位置' }}</div>

            <div class="score-box">
              <div class="score-main">
                <span class="big">{{ (win.scoreAvg ?? 0).toFixed(1) }}</span>
                <span class="of">/ 5.0</span>
                <div class="stars"><StarRating :value="win.scoreAvg ?? 0" /></div>
              </div>
              <div class="score-dims">
                <div class="dim"><span>口味</span><el-rate :model-value="win.tasteAvg ?? 0" disabled allow-half :disabled-void-color="'#e4e7ed'" /></div>
                <div class="dim"><span>性价比</span><el-rate :model-value="win.valueAvg ?? 0" disabled allow-half :disabled-void-color="'#e4e7ed'" /></div>
                <div class="dim"><span>分量</span><el-rate :model-value="win.portionAvg ?? 0" disabled allow-half :disabled-void-color="'#e4e7ed'" /></div>
              </div>
            </div>

            <div class="stats">
              <span><el-icon><ChatDotRound /></el-icon>{{ win.ratingCount || 0 }} 评价</span>
              <span><el-icon><View /></el-icon>{{ win.viewCount || 0 }} 浏览</span>
              <span><el-icon><Goods /></el-icon>{{ win.dishCount || 0 }} 菜品</span>
            </div>

            <div class="actions">
              <el-button type="primary" round @click="openRate"><el-icon><EditPen /></el-icon>&nbsp;写评价</el-button>
              <el-button round :type="win.isLiked ? 'danger' : 'default'" @click="handleLike">
                <el-icon><CaretTop /></el-icon>&nbsp;点赞 {{ likeCount }}
              </el-button>
              <el-button round :type="win.isFavorited ? 'warning' : 'default'" @click="handleFavorite">
                <el-icon><Star /></el-icon>&nbsp;{{ win.isFavorited ? '已收藏' : '收藏' }}
              </el-button>
            </div>
          </div>
        </div>
        ...
        <!-- 菜品列表 -->
        <h3 class="section-title"><el-icon><Food /></el-icon>&nbsp;档口菜品</h3>
        <div v-if="dishes.length" class="rank-list">
          <RankItem v-for="(d, i) in dishes" :key="d.id" :rank="i + 1" :item="d" target="dish" />
        </div>
        <el-empty v-else description="该档口暂无菜品" />
...
import { getWindow, rankDishes, getRatings, toggleLike, toggleFavorite } from '@/api'
...
const loadDishes = async () => {
  const res = await rankDishes({ type: 'overall', windowId: id, page: 1, size: 100 })
  dishes.value = res.data.list || []
}
const loadRatings = async () => {
  const res = await getRatings({ targetType: 'WINDOW', targetId: id, page: 1, size: 50 })
  ratings.value = res.data.records || []
}

const reload = async () => {
  const res = await getWindow(id)
  win.value = res.data
...
const handleLike = async () => {
  ...
  const res = await toggleLike({ targetType: 'WINDOW', targetId: id })
  win.value.isLiked = res.data.liked
  likeCount.value = res.data.count
}
const handleFavorite = async () => {
  if (!requireLogin()) return
  const res = await toggleFavorite({ targetType: 'WINDOW', targetId: id })
  win.value.isFavorited = res.data
}
